...
export class OemQuotesAndVendosService {
   ...
   */
  /**
   * you can use to check all possible sql injection
   * sqlmap -u http://demo.localhost:3000/quotes-and-vendos/all --headers="Authorization: Bearer <your token>" -a --level=5 --risk=3 --no-cast --threads=5 -X pg_catalog,information_schema
   */
  ...
  async getMany(
    ...
    //TODO we should not have a db specific escape
    const page = escape(query.page ? Number(query.page) : 1);
    const limit = escape((query.limit && Number(query.limit)) || 1000);

    const sort = query?.sort || 'updatedAt,DESC';
    const sorts = Array.isArray(sort)
      ? sort.map((s) => this.escape(s).split(','))
      : [this.escape(sort).split(',')];

    const or = query.or || [];
    const ors = Array.isArray(or)
      ? or.map((o) => this.escape(o).split('||'))
      : [this.escape(or).split('||')];
    console.log({ page, limit, ors, sorts });
