...
import { OemNotification } from '../../../main/oem-notifications/oem-notifications/oem-notification.entity';
import { OemNotificationsService } from '../../../main/oem-notifications/oem-notifications/oem-notifications.service';
import { OemNotificationTypeEnum } from '../../../main/oem-notifications/oem-notifications/oem-notification.enums/oem-notification.notification-type.enum';
...
import { OemNotificationPreference } from '../../../main/oem-notifications/oem-notification-preferences/oem-notification-preference.entity';
import { OemNotificationFrequencyType } from '../../../main/oem-notifications/oem-notification-preferences/oem-notification-preference.enums/oem-notification-preference.frequency-type.enum';
...
export class OemQuoteApprovalQueuesService extends TypeOrmCrudService<OemQuoteApprovalQueue> {
  ...
  async rejectedHandler(
    ...
    await manager.update(OemQuoteEntity, currentQuoteApprovalQueue.quoteId, {
    ...
    return this.rejectEvent(req, updatedQuote);
  }

  //only for action logs
  @ActionLogs(ActionLogTypeEnum.QUOTE, ActionsEnum.APPROVE)
  async approveEvent(req, updatedQuote): Promise<OemQuoteEntity> {
    return updatedQuote;
  }

  @ActionLogs(ActionLogTypeEnum.QUOTE, ActionsEnum.TRANSACT)
  async transactEvent(req, updatedQuote): Promise<OemQuoteEntity> {
    return updatedQuote;
  }

  @ActionLogs(ActionLogTypeEnum.QUOTE, ActionsEnum.REJECT)
  async rejectEvent(req, updatedQuote): Promise<OemQuoteEntity> {
    return updatedQuote;
  }
  ...
  async transactedHandler(
    ...
    return await this.transactEvent(req, updatedQuote);
  }
  ...
  async approvedHandler(
    ...
    if (
      ...
      return await this.transactedHandler(req, quoteId, manager);
    } else if (
      ...
      return await this.approveEvent(req, updatedQuote);
    }
  }
  ...
  async validate(
    ...
    const quoteApprovalQueue = await manager
      ...
      .where(
        ...
        // // The customer's role is not relevant for transacting.
...
*/
  ...
  async autoOperateApprovalQueue(
    req: CrudRequest,
    operation:
      | QuoteApprovalQueueStatusEnum.APPROVED
      | QuoteApprovalQueueStatusEnum.REJECTED,
    ...
    if (operation == QuoteApprovalQueueStatusEnum.APPROVED) {
      return await this.approvedHandler(req, quoteApprovalQueue, manager);
    }
    if (operation == QuoteApprovalQueueStatusEnum.REJECTED) {
      return await this.rejectedHandler(req, quoteApprovalQueue, manager);
    }
  }
  ...
  async sendOrQueueApprovalEmail(
    ...
    const dynamicTemplateData: EmailDynamicTemplate = {
      ...
      subject: `${ownerQuoteUser.user.fullName} has submitted a quote for review. The quote will also appear in your Workflow Approval Queue.`,
      subtitle: 'Quote Details / Summary Information',
      body: `
        <table style='min-width:100%;text-align:left;'>
          <tr>
            <td style='text-align:right;'>User Name:</td>
            <td style='font-style:bold;'>${ownerQuoteUser.user.fullName}</td>
          </tr>
          <tr>
            <td style='text-align:right;'Customer Name:</td>
            <td style='font-style:bold;'>${customer.customerName}</td>
          </tr>
          <tr>
            <td style='text-align:right;'>Quote Name:</td>
            <td style='font-style:bold;'>${quote.quoteName}</td>
          </tr>
          <tr>
            <td style='text-align:right;'>Opportunity ID:</td>
            <td style='font-style:bold;'>${quote.opportunityId}</td>
          </tr>
          <tr>
            <td style='text-align:right;'>Net Price: </td>
            <td style='font-style:bold;'>${netAmount}</td>
          </tr>
        </table>
      `.replace(/\s+/g, ' '),
      ...
      This message was sent to ${email} because you're a user in the ${
        quoteApprovalQueue.company?.companyName || 'company'
      } Vendori account.
  ...
  async sendOrQueueApprovedEmail(
    ...
    for (const quoteApprovalQueue of quoteApprovalQueues) {
      ...
      const dynamicTemplateData: EmailDynamicTemplate = {
        logoURL: quoteApprovalQueue.company.logoUrl,
        CTA: confirmationLink,
        subject: `${quoteApprovalQueue.company.companyName} has has sent you a quote to review.`,
        subtitle: 'Quote Details / Summary Information',
        body: `
          <table style='min-width:100%;text-align:left;'>
            <tr>
            ...
            <tr>
              ...
              <td style='font-style:bold;'>${data.quote.customer?.customerName}</td>
            </tr>
            <tr>
              <td style='text-align:right;'>Quote Name:</td>
              <td style='font-style:bold;'>${data.quote.quoteName}</td>
            </tr>
            <tr>
              <td style='text-align:right;'>PIN CODE: </td>
              <td style='font-style:bold;'>${quoteApprovalQueue.quote.pinCode}</td>
            </tr>
          </table>
        `.replace(/\s+/g, ' '),
        subtext: 'Note: The pin code above is required to access your quote.',
        companyAddress:
          quoteApprovalQueue.company?.companyAddress?.address?.city,
        // emailverify: `
        // This message was sent to ${email} because you're a user in the ${
        //   quoteApprovalQueue.company?.companyName || 'company'
        // } Vendori account.
  ...
  async sendOrQueueExpiredEmail(quoteId: number, manager: EntityManager) {
    ...
    for (const quoteUser of quote.usersQuotes) {
      ...
      if (
        (!quote.isExternal && quoteUser.user.isExternal) ||
        !quoteUser.sendNotification
      ) {
        continue;
      }
