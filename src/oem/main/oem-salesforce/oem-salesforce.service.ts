...
import { SalesforceSyncService } from '../../../shared/salesforce/salesforce-sync.service';
import {
...
export class OemSalesforceService {
  ...
  public async setCustomerIdSF(
    ...
    this.salesforceService.setCustomerIdSF(id, sCustomerId, companyId);
  }
  ...
  public async getContacts(params: OemContactListGetDto) {
    ...
    try {
      return await this.salesforceService.getContactList(companyId);
    } catch (error) {
      const isAuthError = await this._handleTokenError(companyId, error);
  ...
  private async _getToken(companyId: number) {
    ...
    const salesforceToken = await this.salesforceService.getTokenObjectSF(
      companyId,
    );
  ...
  private async _handleTokenError(
    ...
    if (errorTypeIsAuthRelated) {
      ...
      const salesforceToken = await this.salesforceService.refreshTokenSF(
        companyId,
      );
