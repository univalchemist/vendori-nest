...
import { OemNotificationTypeEnum } from '../oem-notifications/oem-notifications/oem-notification.enums/oem-notification.notification-type.enum';
import { OemChannelRequestDto } from './oem-channel.dto/oem-channel.request.dto';
import { OemNotificationsService } from '../oem-notifications/oem-notifications/oem-notifications.service';
...
export class OemChannelsService extends TypeOrmCrudService<OemChannelEntity> {
  ...
  async requestChannel(user: any, body: OemChannelRequestDto) {
    ...
    const dynamicTemplateData: EmailDynamicTemplate = {
      ...
      subject: `${user.fullName} from ${user.company?.companyName} has submitted the following channel connection request`,
      subtitle: 'Quote Details / Summary Information',
      body: `
        <br/><br/>
        <table>
          <tr><td>Partner: </td><td>${body.partnerName}</td></tr>
          <tr><td>Website: </td><td>${body.website}</td></tr>
          <tr><td>Contact: </td><td>${body.contactEmail}</td></tr>
        </table>
      `.replace(/\s+/g, ' '),
