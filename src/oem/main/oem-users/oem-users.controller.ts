...
export class OemUsersController implements ICrudControllerCustomDelete<User> {
  ...
  async deleteOne(
    ...
    //TODO: we should improve it, ps. class decorator overrides dto validation
    const dtoObject = plainToInstance(OemUserDeleteDto, dto);
    const error = (await validate(dtoObject))[0];
    if (error) {
      throw new HttpException({ message: error.constraints }, 422);
    }
    return await this.service.deleteOne.call(this, req, dto);
  }

  /**
   * Moved to DELETE user/1?replaceUserId=
   * @deprecated
   */
