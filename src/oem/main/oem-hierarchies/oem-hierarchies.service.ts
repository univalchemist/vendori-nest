...
export class OemHierarchiesService extends TypeOrmCrudService<OemHierarchyEntity> {
  ...
  public async deleteWithReplacement(params: OemHierarchyDeleteDto) {
    ...
    const hierarchy = await this.repo.findOne({
      join: { alias: 'hierarchyLevel' },
      where: { hierarchyId },
    });

    if (!hierarchy) throw new NotFoundException('Hierarchy not found');

    const replacementHierarchy = await this.repo.findOne({
      where: [
        { hierarchyId: replacementHierarchyId },
        {
          isEnabled: true,
          hierarchyLevelId: hierarchy.hierarchyLevel?.hierarchyLevelId,
        },
      ],
    });
