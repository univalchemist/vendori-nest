...
import { Connection, EntityManager, ILike, In, Repository } from 'typeorm';
...
import { OemQuotesUsersService } from '../../intermediaries/_oem-quotes-users/oem-quotes-users.service';
...
export class OemQuotesService extends TypeOrmCrudService<OemQuoteEntity> {
  constructor(
    ...
    @Inject(OemQuotesUsersService)
    private readonly quotesUsersService: OemQuotesUsersService,
    @Inject('BUCKET_NAME')
  ...
  private async _updateQuoteUUID(dto, company): Promise<OemQuoteAndVendoUuid> {
    return this.quoteAndVendoUuidsService.updateUuid(
      UuidTypesEnum.QUOTE,
      dto,
      company,
    );
  ...
  public async _createOne(
    ...
    const uuid = await this._updateQuoteUUID(dto, req.user.company);
    ...
    if (!dto.ownerUserId) dto.ownerUserId = req.user.userId;
  ...
  async createOne(
    ...
    const createdQuote = await this._createOne(req, dto);

    const quote = await this.repo.findOne({
      where: { quoteId: createdQuote.quoteId },
      relations: ['quotesProducts'],
    });

    if (dto.sfMetaData) await this.createDefaultQuoteUsersSF(req, dto, quote);

    // TODO: Move this at some point @salesforce
  ...
  async updateOne(
    ...
    return this.repo.manager.connection.transaction(async (manager) => {
      ...
      // TODO: move to the salesforce-integration endpoint @salesforce
      await this.resetOtherQuotePrimariesSF(dto, quote);
      await this.updateQuoteOwnerSF(dto, quote);

      if (quote.quoteStatus === QuoteStatusEnum.APPROVED)
        dto.approvedDate = new Date();
  ...
  async createDefaultQuoteUsersSF(
    req: any,
    dto: Partial<OemQuoteCreateDto>,
    quote: Partial<OemQuoteEntity>,
  ) {
    await this.quotesUsersService.integrateQuoteContactSF(
      req,
      quote,
      dto.sfMetaData as ISalesforceMetaData,
    );

    return await this.usersQuotesRepo.save(
      this.usersQuotesRepo.create({
  ...
  async resetOtherQuotePrimariesSF(
    ...
    if (quote.quotesProducts.length == 0) return;
    ...
    dto.isPrimary = !dto.isPrimary ? false : true;

    if (quotes.length === 1 && quotes[0].quotesProducts.length > 0)
      dto.isPrimary = true;

    const secondaryQuoteIds = quotes
      .filter((q) => q.isPrimary && q.quoteId !== quote.quoteId)
      .map((q) => q.quoteId);
    const primaryQuoteIds = quotes
      .filter((q) => q.isPrimary)
      .map((q) => q.quoteId)
      .filter((qId) => qId !== quote.quoteId);

    if (dto.isPrimary && secondaryQuoteIds.length > 0) {
      // If we change primary quote, then we need to remove current opportunity product ids in oem-quote-product
      const quoteProducts = await this.repoQuoteProduct.find({
        quoteId: In(secondaryQuoteIds),
      });
      ...
      for (const q of secondaryQuoteIds)
        await this.repo.update(q, { isPrimary: false });
    }

    if (dto.isPrimary) {
      await this.repo.update(
        { quoteId: In(primaryQuoteIds) },
        { isPrimary: false },
      );

      await this.repo.update(quote.quoteId, {
        isPrimary: true,
      });
    }
  }
  ...
  async updateQuoteOwnerSF(
    ...
    // 1. Get all of the opportunity quote users.
    // 2. Separate the curront quote users.
    // 3. Check if the specified owner is added to the current quote. Add them if not
    // 4. Remove ownership for existing users.
    // 5. Update the owner id on all opportunity quotes to the specified owner.
    // 6. Update the ownership for quote user for the current quote.

    if (dto.ownerUserId) {
      const opportunityQuoteUsers = await this.usersQuotesRepo.find({
        where: (qb) => {
          qb.leftJoin('QuotesUsers.quote', 'quote')
            .where({
              quoteId: quote.quoteId,
              isEnabled: true,
            })
            .andWhere('quote.opportunityId = :opportunityId', {
              opportunityId: quote.opportunityId,
            });
        },
      });

      const quoteUsers = opportunityQuoteUsers.filter(
        (u) => u.quoteId === quote.quoteId,
      );
      const quoteUserAdded = quoteUsers.filter(
        (u) => u.userId === dto.ownerUserId,
      );
      ...
      if (!quoteUserAdded.length)
        this.usersQuotesRepo.save(
          this.usersQuotesRepo.create({
            userId: dto.ownerUserId,
            quoteId: quote.quoteId,
            type: QuoteUserTypeEnum.INTERNAL,
      ...
      return await this.usersQuotesRepo.update(
        {
          userId: dto.ownerUserId,
          quoteId: In(opportunityQuoteUsers.flatMap((u) => u.quoteId)),
        },
        { isOwner: true },
      );
    }
