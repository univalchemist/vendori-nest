 ...
 */
//TODO: we can improve it tries to get manager directly instead through repo
  ...
  const decoratedClass = class extends target {
    ...
    constructor(...args: any[]) {
      ...
      for (const [propertyName, descriptor] of Object.entries(descriptors)) {
        ...
        if (
          /* propertyName.indexOf('_') === 0 ||*/ // we use a convention that all methods which start from '_' are private/static
          !isAsyncFunction(descriptor.value) ||
          propertyName.indexOf('defaultMessage') !== -1
        )
          continue;
