...
import CreateDemoOemCompanyChannelSettings from '../../src/oem/seeds/create-oem-company-channel-settings.seed';
...
describe('QuoteAndVendos (e2e)', () => {
  ...
  beforeAll(async (done) => {
    ...
    await runSeeder(CreateDemoOemCompanyChannelSettings(channels));
    ...
    //create internal quote3 user
    await factory(OemQuotesUsers)().create({
      userId: user.userId,
      quoteId: quote_3.quoteId,
      isOwner: true,
      isApprover: false,
      type: QuoteUserTypeEnum.INTERNAL,
      companyId: 1,
    ...
    /*const manager = getConnection('MASTER_CONNECTION_CONF').manager;

    console.log(await manager.find(
      OemHierarchyEntity
    ));*/
    ...
    it(`should ${getMetaData(method).action} a ${MODEL}`, async () => {
      const res = await request(server)
        [method](
          PATH +
            '/' +
            'all?sort=quoteId%2CDESC&sort=vendoId%2CDESC&or=quoteUuid%7C%7C%24contL%7C%7CQ-1234567891',
        )
        .set('Origin', 'demo.localhost')
        .auth(access_token, { type: 'bearer' });
      console.debug(res.body);
