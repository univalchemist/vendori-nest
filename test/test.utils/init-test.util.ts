...
export function initTestDescription(params: ITest) {
  return function (
    comparedData?: ComparedData | object | Promise<ComparedData & object>,
            ...
            (promises) => {
              ...
              res.end(async (_, res) => {
                ...
                expect(res.status).toBe(
                  (comparedData as ComparedData)?.status ||
                    params.status ||
                    getMetaData(params.method).expectedStatus,
                );

                const comparedDataBody =
                  (comparedData as ComparedData)?.body || comparedData;

                if (comparedDataBody && Array.isArray(res.body.data)) {
                  expect(res.body.data).toEqual(
                    expect.arrayContaining([
                      expect.objectContaining(
                        params.SerializeClass
                          ? new params.SerializeClass(comparedDataBody)
                          : comparedDataBody,
                      ),
                    ]),
                  );
